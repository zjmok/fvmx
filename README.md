...
- [x] `fvmx use <repo@ref>`：在当前项目创建 `.fvmx/flutter_sdk` 指向已安装 SDK，并写入 `.fvmx/version` 与 `.fvmxrc`
- [x] `fvmx flutter [args...]`：转发到当前项目 `.fvmx/flutter_sdk/bin/flutter`
- [x] `fvmx remove <repo@ref>`：通过 `git worktree remove` 删除已安装版本
- [x] 基础测试：覆盖第一阶段的完整本地 Git 流程
